...
open class HACycleScrollView: UIView ,UIScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate{
    ...
    // 加载状态图
    fileprivate var placeHolderViewImage: UIImage! = UIImage(named: "HACycleScrollView.bundle/llplaceholder.png", in: Bundle(for: HACycleScrollView.self), compatibleWith: nil)
    
    // 空数据页面显示占位图
    fileprivate var coverViewImage: UIImage! = UIImage(named: "HACycleScrollView.bundle/llplaceholder.png", in: Bundle(for: HACycleScrollView.self), compatibleWith: nil)
    ...
    // MARK: 初始化
    open class func llCycleScrollViewWithFrame(_ frame: CGRect, imageURLPaths: Array<String>? = [], titles:Array<String>? = [],didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> HACycleScrollView {
        let llcycleScrollView: HACycleScrollView = HACycleScrollView.init(frame: frame)
    ...
    // MARK: 纯文本
    open class func llCycleScrollViewWithTitles(frame: CGRect, backImage: UIImage? = nil, titles: Array<String>? = [], didSelectItemAtIndex: LLdidSelectItemAtIndexClosure? = nil) -> HACycleScrollView {
        let llcycleScrollView: HACycleScrollView = HACycleScrollView.init(frame: frame)
    ...
    private func setupMainView() {
        ...
        collectionView.register(HACycleScrollViewCell.self, forCellWithReuseIdentifier: identifier)
    ...
    func setupPageControl() {
        ...
        if customPageControlStyle == .fill {
            customPageControl = HAFilledPageControl.init(frame: CGRect.zero)
            customPageControl?.tintColor = customPageControlTintColor
            (customPageControl as! HAFilledPageControl).indicatorPadding = customPageControlIndicatorPadding
            (customPageControl as! HAFilledPageControl).indicatorRadius = FillPageControlIndicatorRadius
            (customPageControl as! HAFilledPageControl).pageCount = self.imagePaths.count
            self.addSubview(customPageControl!)
        }
        
        if customPageControlStyle == .pill {
            customPageControl = HAPillPageControl.init(frame: CGRect.zero)
            (customPageControl as! HAPillPageControl).indicatorPadding = customPageControlIndicatorPadding
            (customPageControl as! HAPillPageControl).activeTint = customPageControlTintColor
            (customPageControl as! HAPillPageControl).inactiveTint = customPageControlInActiveTintColor
            (customPageControl as! HAPillPageControl).pageCount = self.imagePaths.count
            self.addSubview(customPageControl!)
        }
        
        if customPageControlStyle == .snake {
            customPageControl = HASnakePageControl.init(frame: CGRect.zero)
            (customPageControl as! HASnakePageControl).activeTint = customPageControlTintColor
            (customPageControl as! HASnakePageControl).indicatorPadding = customPageControlIndicatorPadding
            (customPageControl as! HASnakePageControl).indicatorRadius = FillPageControlIndicatorRadius
            (customPageControl as! HASnakePageControl).inactiveTint = customPageControlInActiveTintColor
            (customPageControl as! HASnakePageControl).pageCount = self.imagePaths.count
            self.addSubview(customPageControl!)
        }
    }
    // MARK: layoutSubviews
    override open func layoutSubviews() {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system {
            if pageControlPosition == .center {
                pageControl?.frame = CGRect.init(x: 0, y: self.H-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
            }else{
                let pointSize = pageControl?.size(forNumberOfPages: self.imagePaths.count)
                if pageControlPosition == .left {
                    pageControl?.frame = CGRect.init(x: -(UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: self.H-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }else{
                    pageControl?.frame = CGRect.init(x: (UIScreen.main.bounds.width - (pointSize?.width)! - pageControlLeadingOrTrialingContact) * 0.5, y: self.H-pageControlBottom, width: UIScreen.main.bounds.width, height: 10)
                }
            }
        }else{
            var y = self.H-pageControlBottom
    ...
    func currentIndex() -> NSInteger {
        if collectionView.W == 0 || collectionView.H == 0 {
            return 0
        }
        var index = 0
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell: HACycleScrollViewCell = collectionView.dequeueReusableCell(withReuseIdentifier: identifier, for: indexPath) as! HACycleScrollViewCell
        // Setting
        cell.titleFont = font
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        if customPageControlStyle == .none || customPageControlStyle == .system {
            ...
            // progress
            if customPageControlStyle == .fill {
                (customPageControl as! HAFilledPageControl).progress = progress
            }else if customPageControlStyle == .pill {
                (customPageControl as! HAFilledPageControl).progress = progress
            }else if customPageControlStyle == .snake {
                (customPageControl as! HASnakePageControl).progress = progress
            }
        }
